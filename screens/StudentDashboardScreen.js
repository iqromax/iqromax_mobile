...
export default function StudentDashboardScreen({ navigation, route }) {
  ...
  const renderKiyimGrid = () => {
    ...
    const shopInventoryItems = (shopItems || []).filter(item => item.category === 'inventory').map(item => {
      ...
      const catMap = { headwear: 'bosh_kiyim', top: 'ustki_kiyim', tshirt: 'futbolka', pants: 'shim', shoes: 'oyoq_kiyim', accessories: 'aksessuar', backpacks: 'ryukzak' };
      const category = catMap[sub] || (sub === 'bosh_kiyim' ? 'bosh_kiyim' : sub === 'ustki_kiyim' ? 'ustki_kiyim' : sub === 'futbolka' ? 'futbolka' : sub === 'shim' ? 'shim' : sub === 'oyoq_kiyim' ? 'oyoq_kiyim' : sub === 'aksessuar' ? 'aksessuar' : sub === 'ryukzak' ? 'ryukzak' : 'bosh_kiyim');
      return {
        ...item,
        category,
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <View style={[styles.mainContent, { display: activeTab === 'exercise' ? 'flex' : 'none' }]}>
        ...
        <ScrollView style={{ flex: 1 }} nestedScrollEnabled={true} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: Platform.OS === 'android' ? 275 : 265, paddingBottom: 170, paddingHorizontal: 20 }}>
        ...
        <View style={{ position: 'absolute', bottom: 80 + (Platform.OS === 'android' ? Math.max(insets.bottom, 15) : insets.bottom), left: 0, right: 0, paddingHorizontal: 20, paddingTop: 8, paddingBottom: 6, backgroundColor: '#05050C', zIndex: 50, borderTopWidth: 1, borderTopColor: 'rgba(255, 255, 255, 0.05)' }}>
          ...
          {activeExerciseType === 'battle' ? (
            ...
            <TouchableOpacity 
              style={[styles.startExerciseBtn, (activeExerciseType === 'abacus' || activeExerciseType === 'speed') && { marginTop: 15 }]}
              activeOpacity={0.8}
