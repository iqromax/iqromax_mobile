...
function AccessoryModel({ modelPath, yPos, characterIndex, isHeadwear = false }) {
  ...
  const accessoryScale = isHeadwear ? 25.0 : 0.50;
  
  let xOffset = 0;
  let heightOffset = isHeadwear ? 1.0 : 2.10; // Position right in middle of chest/head area
  let zOffset = isHeadwear ? 0.5 : 0.20;
  
  if (characterIndex === 0) { // Athletic Man
    heightOffset = isHeadwear ? 1.2 : 1.50; 
  } else if (characterIndex === 1) { // Adult Male
    heightOffset = isHeadwear ? 1.0 : 1.50;
  } else if (characterIndex >= 4) { // Qizlar
    heightOffset = isHeadwear ? 1.2 : 2.05; 
    zOffset = isHeadwear ? 0.5 : 0.30; 
  }
...
function CharacterModel({ characterIndex, yOffset = 0, accessoryPath = null, headwearPath = null, isHeadwear = false }) {
  ...
  // Fix Android GL materials: Convert materials to MeshBasicMaterial on Android so textures render clearly without WebGL shader lighting bugs, and MeshLambertMaterial on iOS
  React.useMemo(() => {
              ...
              <ScrollView style={{ maxHeight: 400, width: '100%' }} showsVerticalScrollIndicator={false}>
                {notificationsList.length === 0 ? (
                  ...
                  notificationsList.map((notif, idx) => {
                    ...
                    } else if (notif.type === 'PARENT_INVITE') {
                      let inviteData = {};
                      try { inviteData = JSON.parse(notif.message); } catch (e) {}
                      const parentName = inviteData.parentName || "Ota-onangiz";
                      return (
                        <View key={notif.id || idx} style={styles.notifItemCard}>
                          <View style={styles.notifItemHeader}>
                            <View style={[styles.notifItemAvatar, { backgroundColor: 'rgba(59, 130, 246, 0.2)', justifyContent: 'center', alignItems: 'center' }]}>
                              <MaterialCommunityIcons name="account-child-circle" size={26} color="#3B82F6" />
                            </View>
                            <View style={{ flex: 1 }}>
                              <Text style={styles.notifItemSender}>👨‍👩‍👦 Ota-onangiz siz bilan bog'lanmoqchi</Text>
                              <Text style={styles.notifItemStats}>👤 {parentName}</Text>
                            </View>
                            <View style={[styles.notifBadge, { backgroundColor: 'rgba(59, 130, 246, 0.2)', borderColor: '#3B82F6' }]}>
                              <Text style={[styles.notifBadgeText, { color: '#3B82F6' }]}>Ota-ona</Text>
                            </View>
                          </View>
                          <Text style={styles.notifItemMsg}>
                            <Text style={{ fontWeight: 'bold', color: '#60A5FA' }}>{parentName}</Text> sizning o'quv faoliyatingizni kuzatish uchun ota-ona hisobiga ulanishni so'radi.
                          </Text>
                          <View style={styles.notifItemActions}>
                            <TouchableOpacity 
                              style={styles.notifRejectBtn} 
