...
const Users = () => {
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setUsers(prev => prev.map(s => s.id === editingUser.id ? editingUser : s));
        setEditingUser(null);
      } else {
      ...
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              Umumiy foydalanuvchilar ro'yxati
            </h1>
            <p className="text-sm text-indigo-200/60 mt-1">
              Barcha foydalanuvchilarni boshqarish va kuzatish (O'quvchi, O'qituvchi, Ota-ona)
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#0A0A16] border border-[#1A1A2F] rounded-2xl shadow-lg overflow-hidden">
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              ...
              <thead>
                <tr className="border-b border-[#1A1A2F] bg-[#05050C]/50">
                  ...
                  <th className="py-4 px-6 text-xs font-semibold text-indigo-200/50 uppercase tracking-wider">Rol</th>
                ...
                {isLoading ? (
                  ...
                  filteredUsers.map((user) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 text-sm text-indigo-100/80 whitespace-nowrap">
                        {getCountryFullName(user.country)}
                      </td>
          ...
          <div className="border-t border-[#1A1A2F] bg-[#05050C]/50 px-6 py-4 flex items-center justify-between">
            <span className="text-xs text-indigo-200/50">Jami {filteredUsers.length} ta foydalanuvchi ko'rsatilmoqda</span>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0A0A16] border border-[#1A1A2F] rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <h2 className="text-xl font-bold text-white mb-6">Foydalanuvchini tahrirlash</h2>
            <form onSubmit={handleEditSave} className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-indigo-200/60 mb-1">Rol</label>
                <select 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[#0A0A16] border border-[#1A1A2F] rounded-2xl shadow-[0_0_40px_rgba(239,68,68,0.15)] overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Foydalanuvchini o'chirish</h3>
              <p className="text-sm text-indigo-200/70 mb-6 leading-relaxed">
                Haqiqatan ham <strong className="text-white font-semibold">{userToDelete.name}</strong> ismli foydalanuvchini o'chirmoqchimisiz? Bu amalni ortga qaytarib bo'lmaydi.
              </p>
